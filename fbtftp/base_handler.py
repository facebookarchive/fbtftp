...
class BaseHandler(multiprocessing.Process):
    ...
    def __init__(self, server_addr, peer, path, options, stats_callback):
        ...
        self._peer = peer
        logging.info("New connection from peer `%s` asking for path `%s`" %
                     (str(peer), str(path)))
        family = socket.AF_INET6
        # the format of the peer tuple is different for v4 and v6
        if isinstance(ipaddress.ip_address(server_addr[0]),
    ...
    def _close(self, test=False):
        ...
        try:
            ...
            logging.exception(
                'Exception raised when calling _on_close: %s' % e)
        finally:
    ...
    def _parse_options(self):
        ...
        logging.info('Options requested from peer {}:  {}'.format(
            self._peer, self._options))
        ...
        logging.info('Options to ack for peer {}:  {}'.format(
            self._peer, self._options))
    ...
    def on_new_data(self):
        ...
        if code == constants.OPCODE_ERROR:
            ...
            logging.error("Error reported from client: %s" %
                          self._stats.error['error_message'])
            self._transmit_error()
            self._should_stop = True
            return
        if code != constants.OPCODE_ACK:
            logging.error('Expected an ACK opcode from %s, got: %d' % (
                self._peer, code))
            self._stats.error = {
                'error_code': constants.ERR_ILLEGAL_OPERATION,
    ...
    def _handle_timeout(self):
        ...
        self._stats.error = {
            'error_code': constants.ERR_UNDEFINED,
            'error_message': 'timeout after %d retransmits' %
            self._retransmits,
        }
        self._should_stop = True
    ...
    def _transmit_data(self):
        ...
        packet = struct.pack(fmt, constants.OPCODE_DATA,
                             self._last_block_sent,
                             self._current_block)
    ...
    def _transmit_oack(self):
        ...
        for key, val in self._options.items():
            ...
            opts.append(struct.pack(
                fmt,
                bytes(key.encode('latin-1')),
                bytes(val.encode('latin-1'))))
        opts.append(b'')
        fmt = str('!H')
    ...
    def _transmit_error(self):
        ...
        fmt = str('!HH%dsx' % (
            len(self._stats.error['error_message'].encode('latin-1'))))
        packet = struct.pack(
