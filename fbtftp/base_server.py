...
class BaseServer(object):
    ...
    def restart_stats_timer(self, run_once=False):
        ...
        if self._server_stats_callback is None:
            logging.warning('No callback specified for server statistics '
                            'logging, will continue without')
            return
        self._metrics_timer = threading.Timer(
            self._server_stats.interval,
            self._metrics_callback_wrapper,
            [run_once]
        )
        logging.debug('Starting the metrics callback in {sec}s'.format(
    ...
    def on_new_data(self):
        ...
        if code != constants.OPCODE_RRQ:
            logging.warn('unexpected TFTP opcode %d, expected %d' % (
        ...
        if len(tokens) < 2 or len(tokens) % 2 != 0:
            logging.error('Received malformed packet, ignoring '
