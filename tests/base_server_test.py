...
class testBaseServer(unittest.TestCase):
    ...
    def prepare_and_run(self, network_queue):
        server = StaticServer(self.host, self.port, self.retries, self.timeout,
                              None, Mock(), self.interval, self.network_queue)
    ...
    def testRRQ(self, epoll_mock):
        ...
        self.assertEqual(handler.options,
                         {'default_timeout': 100, 'mode': 'binascii',
                          'opt1_key': 'opt1_val', 'retries': 200})

    def start_timer_and_wait_for_callback(self, stats_callback):
        server = StaticServer(self.host, self.port, self.retries, self.timeout,
                              None, stats_callback, self.interval, [])
