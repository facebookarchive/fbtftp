...
class testSessionHandler(unittest.TestCase):
    ...
    def setUp(self):
        self.options = {'default_timeout': 10,
                        'retries': 2,
                        'mode': 'netascii',
                        'blksize': 1492,
                        'tsize': 0,
    ...
    def testResponseDataException(self):
        ...
        with patch.object(MockHandler, 'get_response_data') as mock:
            ...
            self.assertEqual(handler._stats.error,
                             {'error_message': 'boom!', 'error_code': 0})

    def testParseOptionsNetascii(self):
        self.handler._response_data = StringResponseData("foo\nbar\n")
        self.handler._parse_options()
        self.assertEqual(self.handler._stats.options_in,
                         {'mode': 'netascii',
                          'blksize': 1492,
                          'tsize': 0,
                          'timeout': 99,
    ...
    def testParseOptionsBadMode(self):
        options = {'default_timeout': 10, 'retries': 2,
                   'mode': 'IamBadAndIShoudlFeelBad', 'blksize': 1492,
                   'tsize': 0, 'timeout': 99}
        self.handler = MockHandler(
            server_addr=self.server_addr,
            peer=self.peer,
        ...
        self.assertTrue(
            self.handler._stats.error[
                'error_message'].startswith('Unknown mode:'))
    ...
    def testOnNewDataHandleAck(self):
        ...
        self.handler._listener.sendto.assert_called_with(
            # client acknolwedges DATA block 1, we expect to send DATA block 2
            b'\x00\x03\x00\x02foo', ('127.0.0.1', 5678))
    ...
    def testTransmitOACK(self):
        ...
        self.handler._listener.sendto.assert_called_with(
            # OACK code == 6
            b'\x00\x06opt1\x00value1\x00', ('127.0.0.1', 5678)
        )
