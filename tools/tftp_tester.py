...
class Spinner:
    ...
    def show(self):
        print(u"\r{0}".format(self.spin()), end="")
        sys.stdout.flush()
...
class TftpTester(object):
    ...
    def __init__(self, server, port, timeout, retries, filename, blksize,
                    failsend, failreceive, verbose):
        self.server = server
        self.port = int(port)
    ...
    def send_and_expect(self, packet, expect, cur):
        ...
        while retries < self.retries:
            ...
            try:
                ...
                is_last = get_packet_type(answer) == TFTP.DATA and \
                        len(get_packet_data(answer)) < self.actual_blksize
                ...
                if get_packet_type(answer) == expect:
                    if (expect == TFTP.DATA and num == cur + 1) \
                            or (expect == TFTP.OACK):
                        break
...
if __name__ == '__main__':
    ...
    parser.add_argument('--server', default='::1', help='server IP address '
                        '(default: ::1)')
    parser.add_argument('--port', default=69, help='server tftp port '
                        '(default: udp/69)')
    parser.add_argument('--timeout', default=5, help='timeout interval in seconds '
                        '(default: 5)')
    parser.add_argument('--retries', default=5, help='number of retries '
                        '(default: 5)')
    parser.add_argument('--filename', required=True, help='remote file name')
    parser.add_argument('--blksize', default=1228, help='block size in bytes '
                        '(default: 1228)')
    parser.add_argument('--failreceive', default=[], help='list of packets which '
                        'will be ignored', nargs='+')
    parser.add_argument('--failsend', default=[], help='list of packets which '
                        'will not be sent', nargs='+')
    parser.add_argument('--verbose', '-v', action='count', help='display a spinner')

    args = parser.parse_args(sys.argv[1:])

    verbose = bool(args.verbose)
    t = TftpTester(server=args.server, port=args.port, filename=args.filename,
                    blksize=args.blksize, timeout=args.timeout, retries=args.retries,
                    failsend=args.failsend, failreceive=args.failreceive,
                    verbose=verbose)
    try:
        t.set_socket()
        t.loop()
